...
export default function InfiniteVideoList({ order, title, showFilter = true }: InfiniteVideoListProps) {
  ...
  }, [currentPage, totalPages, order, loadingMore, showLoadMoreButton, loadCategories])
    ...
    if (nextPage <= totalPages) {
      try {
        ...
        if (res && res.length > 0) {
          ...
          // Mettre à jour l'URL pour le SEO si on est sur une page avec pathname
          if (pathname) {
  ...
  }, [currentPage, totalPages, order, router, pathname])
